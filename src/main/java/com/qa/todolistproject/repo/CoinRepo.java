...
public interface CoinRepo extends JpaRepository<Coin, Long> {
	...
	@Query(value = "SELECT * FROM coin ORDER BY country ASC, denomination ASC, year ASC", nativeQuery = true)
...
//
	@Query(value = "SELECT * FROM coin WHERE in_collection = true ORDER BY country ASC, denomination ASC, year ASC", nativeQuery = true)
